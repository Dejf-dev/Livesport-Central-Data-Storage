...
class Player
{
    ...
    public static function givePlayers(Team $homeTeam, Team $awayTeam): array
    {
        ...
        for ($i = 0; $i < count($playerNames); $i++) {
            $isPlaying = $i < 22;
            $isHomePlayer = $i < 11 || ($i > 21 && $i < 22 + ceil((count($playerNames) - 22) / 2));
     ...
     */
    public static function givePlayerOnTeam(array $players, Team $team, bool $isOnBench, int $time): Player
    {
        $activePlayers = array_values(array_filter($players, fn(Player $player) =>
                                                  ($isOnBench ? $player->getMinuteOnBench() <= $time
                                                  : $player->getMinuteOnBench() == 91)
                                                  && $player->getTeam() === $team
                                                  && $player->getExclusionMinute() > $time));
